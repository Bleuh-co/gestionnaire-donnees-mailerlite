...
/**
 * roleMapper : branche la résolution de rôle propre au Gestionnaire données
 * MailerLite dans le SDK — règles EXACTES de l'ancien getSession, zéro
 * régression :
 *   1. filtre de domaine (isEmailDomainAllowed) → sinon "blocked",
 *   2. resolveRole (bootstrap → user_app_roles → users.role legacy → membre).
